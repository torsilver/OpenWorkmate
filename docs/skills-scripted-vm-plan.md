...
**落地实现**：在 Cursor 中打开与本仓库关联的 **Plan**（`Skills 脚本化 VM — Taskly 落地`），按其中 To-dos 分阶段执行。

**调试与轨迹**：脚本式 Skill 调试器（CLI、轨迹 schema、宿主调试 API）见 [skill-script-debugger-plan.md](skill-script-debugger-plan.md) 与作者操作说明 [skill-debug-authoring.md](skill-debug-authoring.md)。
...
典型场景：**上下文窗口很小的本地部署模型**仍要处理复杂任务，因此 **skill 总体会非常详细**；脚本化分段的目的不是削减作者可写的细节，而是把 **细节留在磁盘与宿主状态中**，每轮只向模型暴露 **当前段 + 短快照**。若仅做分段披露、却不控制 **对话历史膨胀**，会出现「skill 未撑爆窗口，但历史把窗口撑满」的假胜利——因此下列项在本地模型场景下与「分段协议」**同等优先**，实现时不应降级为可选项。

1. **窗口硬上限与每轮注入预算**  
   与宿主侧的上下文上限配置（例如应用内的 `ContextWindow` / 模型 `ContextLength`）对齐，显式约定：**每轮注入预算** ≈ 用户目标 + **状态快照**（短）+ **当前段**（建议设 **单段最大字符或 token 上限**，可配置）+ 工具说明 + **截断后的近期对话**。允许 skill 在总量上极细，但 **单段**不得在无上限情况下占满整窗。

2. **指令遵循较弱时的宿主责任**  
   在本地模型上，「模型不遵守控制流」的风险更高：应默认 **仅在决策 JSON 合法时推进 PC**；非法时重试或要求重读当前段；必要时 **收窄每轮暴露的工具集合**（例如仅 `skill_step`/navigate 类 + 与本段相关的工具），降低选错工具概率。

3. **工具数量与描述长度**  
   并列工具越多，system 中工具描述占用的 token 越多。对本地小窗口，**合并为单一 `skill_step(action, ...)`**（见下文「与『少 MCP』的权衡」）往往比「两个 MCP」更值得优先尝试。

4. **评测指标**  
   除 next/goto/子 skill 的 **黄金路径** 外，应在目标 `MaxContextTokens`（或等价配置）下观测：**多轮对话的估计 token 曲线**（确保 skill 全文从不整段进窗、且历史经裁剪/摘要后仍可控）；并覆盖 **控制流违规后的恢复路径**（非法决策后不损坏状态、可重试）。

**一句话原则**：**详细是总量；渐进是每轮可见量。**
...
**第一期（并列推进，缺一不可）**：下列三项与「仅写分段规范」同等优先——只做其中一项无法充分服务 **本地小上下文** 场景（见上文「面向本地小上下文模型的附加约束」）。

1. **Skill 分段规范**：manifest（skill id、段列表、每段 id、可选元数据）；作者指南中写明 **编排层/执行层** 分工；**单段长度上限**与宿主上下文策略挂钩，避免单段占满整窗。
2. **会话状态（Skill VM / MCP 后端）**：内存或文件后端，维护 `session_id -> { skill, segment_index, stack, variables, checkpoints, artifact_refs }`。
3. **宿主 prompt + 历史策略**：决策 JSON 协议；每轮注入 **目标 + 状态快照 + 当前段**；实现 **历史窗口 + 条件摘要**（与分段披露配套，避免历史单独撑爆窗口）。

**第二期**：显式生命周期（`finish` / `pause`）、检查点持久化、可选 **同 session 恢复** 或跨会话导入快照。

**第三期（验收）**：见下节 **「验收与指标（本地模型）」**。

---

## 验收与指标（本地模型）

在宿主配置的 **目标上下文上限**（如 `MaxContextTokens` 或模型 `ContextLength`）下验证：

- **黄金路径**：顺序 `next`、条件 `goto`、子 skill（若有）、**中断后从检查点恢复**。
- **控制流违规恢复**：故意给出非法决策 JSON 或越权跳转时，宿主 **不损坏 PC/状态**，可重试或回退到「重读当前段」。
- **Token 曲线**：长任务多轮对话下，用与宿主一致的 **估计 token** 方法观测曲线——**skill 全文不应整段进入单轮上下文**；历史经裁剪/摘要后增长可控；若曲线在任务完成前触顶，须归因于「段过长」或「历史策略不足」并迭代。
...
- [ ] **acceptance-local**：在 ContextWindow / 等价配置下测 **估计 token 曲线**、黄金路径与 **控制流违规恢复**（见上文「验收与指标（本地模型）」）

---

## 总结

- **面向本地小上下文 + 详细 Skill**：脚本化分段让 **总量**可极细、**每轮可见量**可控；若不同时做 **历史策略与段外状态**，本地模型仍易被对话历史拖垮（见「面向本地小上下文模型的附加约束」）。
