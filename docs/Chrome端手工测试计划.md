...
| E18 | `excel_hyperlink_set`            | E01 | 「请 excel_hyperlink_set：文件 taskly-excel-test.xlsx，Sheet1，单元格 F1，url 填 https://example.com，displayText【测试链接】。」 | `excel_hyperlink_set`            | 成功               |
...
**读工具接地（可选回归）**：在多轮对话后（助手已口头描述过幻灯片内容），再单独发一条与 **P03** 相同话术「请 ppt_slide_read：…」。若模型首轮未调工具且**已有可见正文**（非仅 thinking），界面可能出现「首轮响应未执行读类文档工具；正在自动续跑一轮…」；若首轮仅有 thinking、无正文则**静默**续跑；第二轮应实际调用 `ppt_slide_read`；后端日志可搜 `Tool grounding retry (read)`。
