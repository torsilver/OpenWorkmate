...
| TL4 | 工具块耗时    | 执行耗时 >1s 的工具（如 `page_agent`）                        | 绿色工具块 summary 上可出现 **「已执行 Ns」** 或输出区「已耗时 Ns」（单工具粒度，保留）                                                                                                                                           |
| TL5 | HITL 确认  | 触发需确认的 `page_agent` / `run_command` 等               | 出现 **顶栏遮罩**；遮罩内可有 **「等待确认 · 已 N 秒…」** 与超时说明。**不**要求时间线内出现「待确认」行（当前为刻意设计；若产品改为写入时间线需另改代码与本文）                                                                                                      |
...
| B03 | `page_agent`        | 同上           | 「请 page_agent：requestJson 为 {\"op\":\"observe\"}。」    | `page_agent`        | 返回 JSON，含 nodes 与 ref        |
...
| B06 | `page_agent`        | 含按钮的普通网页      | 「先 page_agent observe；再对返回中第一个 ref 调用 page_agent，requestJson 为 {\"op\":\"click\",\"ref\":\"r0\"}（ref 以 observe 为准）。」              | `page_agent`        | 成功 JSON 或业务页反馈        |
...
**说明（白名单）**：设置「安全与确认 → Chrome」中 **page_agent op 白名单** 默认与后端 `DefaultAllowedPageAgentOps`（当前为 `observe`）对齐；`click`/`fill` 等需在设置中勾选对应 op 或经 HITL「加入白名单」后再测。
...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Browser           | `highlight_webpage_text`, `add_floating_note`, `page_agent`, `run_custom_javascript_in_page`, `capture_full_page`         |
