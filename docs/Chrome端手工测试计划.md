...
**读工具接地（可选回归）**：在多轮对话后（助手已口头描述过幻灯片内容），再单独发一条与 **P03** 相同话术「请 ppt_slide_read：…」。若模型首轮未调工具，界面可能出现提示「首轮响应未执行读类文档工具；正在自动续跑一轮…」，且第二轮应实际调用 `ppt_slide_read`；后端日志可搜 `Tool grounding retry (read)`。
