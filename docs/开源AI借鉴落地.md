...
本文档落实「开源 AI 工具调研」计划中的三项结论：**优先聚焦主线**、**可汉化提示词模式（映射到本仓库工具）**、**Semantic Kernel 侧与 LangGraph 式状态机的对照评估**。  
不修改计划原文；与 [提示词清单.md](./提示词清单.md)、[PROJECT_PLAN.md](./PROJECT_PLAN.md) 配合阅读。
...
| **P1** | **流式与取消** | `StreamCancelService`、`stop` 消息与 `GetStreamingChatMessageContentsAsync` 已贯通；继续打磨中断与状态提示即可。 |
...
| 概念 | 实现位置 |
|------|-----------|
| interrupt | `SecurityFilter` + `HitlManager`（需用户确认的操作） |
...
**Taskly 映射**：CLI / 系统类工具继续与 `SecurityFilter`、HITL 配置联动；提示词侧可强调：「对可能修改文件或执行命令的操作，先确认范围再执行。」
...
## 3. Semantic Kernel 与 LangGraph 状态机：是否需要「轻量 Planner 状态」（sk-patterns）

### 3.1 当前架构中已覆盖的「状态」

| 能力 | 代码/模块 | 说明 |
|------|-----------|------|
| 会话与对话历史 | `SessionState` + `ChatHistory` | 多轮对话与摘要、`TrimHistory` 等 |
| 工具范围裁剪 | `ToolSelectionService` / 向量索引 + `FunctionChoiceBehavior.Auto(selectedFunctions)` | 降低无关工具干扰 |
...
若引入轻量状态，建议形态为：**会话级 small DTO**（枚举阶段 + 计数器 + 可选 `planId` 指针），在 `ChatService.StreamChatAsync` 入口维护，而不是引入第二套 Agent 运行时。

### 3.3 与 LangGraph 的对应关系（概念图）

```mermaid
flowchart TB
  subgraph taskly [Taskly_SK_当前形态]
    TS[ToolSelection]
    SKStream[GetStreamingChatMessageContentsAsync_plus_tools]
    Plan[PlanInject_and_PlanPlugin]
    HITL[SecurityFilter_HitlManager]
    Sub[RunSubtaskAsync]
  end
...
- [microsoft/semantic-kernel](https://github.com/microsoft/semantic-kernel) — 与当前栈一致。
...
2. 在 **P0 HITL** 上：核对敏感操作是否均走 `SecurityFilter` / 配置，避免静默执行。
