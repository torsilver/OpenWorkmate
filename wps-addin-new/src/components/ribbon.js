...
function OnAction(control) {
  ...
  switch (eleId) {
    ...
    case 'btnShowTaskPane':
      ...
      break
    case 'btn_open_taskpane':
      {
        let copilotTsId = window.Application.PluginStorage.getItem('office_copilot_taskpane_id')
        if (!copilotTsId) {
          let tskpane = window.Application.CreateTaskPane(Util.GetUrlPath() + Util.GetRouterHash() + '/taskpane')
          let id = tskpane.ID
          window.Application.PluginStorage.setItem('office_copilot_taskpane_id', id)
          tskpane.Visible = true
        } else {
          let tskpane = window.Application.GetTaskPane(copilotTsId)
          tskpane.Visible = true
        }
      }
      break
