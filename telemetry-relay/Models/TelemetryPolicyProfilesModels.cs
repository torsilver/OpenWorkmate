...
/// <summary><c>DataRoot/telemetry-relay-policy.json</c> 内 <c>policyProfiles</c> 字段形状：多组策略，每组为启用的 log 种类列表。</summary>
...
public sealed class TelemetryPolicyProfileEntry
{
    ...
    public List<TelemetryLogKindEntry> LogKinds { get; set; } = new();
...
public static class TelemetryPolicyProfilesDefaults
{
    ...
    {
        var kinds = TelemetryLogKindPolicyDefaults.BuiltInKinds
            .Select(k => new TelemetryLogKindEntry { Kind = k.Kind, Label = k.Label })
            .ToList();
    ...
    public static TelemetryPolicyProfilesFile Merge(TelemetryPolicyProfilesFile? fromFile)
    {
        ...
        foreach (var p in fromFile.Profiles)
        {
            ...
            var kinds = new List<TelemetryLogKindEntry>();
            foreach (var e in p.LogKinds ?? [])
            {
                var k = (e.Kind ?? "").Trim();
                if (string.IsNullOrEmpty(k)) continue;
                kinds.Add(new TelemetryLogKindEntry { Kind = k, Label = string.IsNullOrWhiteSpace(e.Label) ? null : e.Label.Trim() });
            }
