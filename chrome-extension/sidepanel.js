...
/** stream_role / stream_meta：主时间线与正文同级，按 blockSeq 插入。（stream_usage 仅圆环；stream_finish 不展示） */
...
// agent_phase→计划·意图 / 处理工具结果；stream_chunk→助手回复；stream_usage→仅输入区圆环；stream_finish→忽略；stream_role|stream_meta→时间线；stream_warning→服务端提示；subtask_* / tool_invocation_*→子任务与工具块。
