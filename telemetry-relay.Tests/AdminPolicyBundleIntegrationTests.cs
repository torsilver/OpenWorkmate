...
public sealed class AdminPolicyBundleIntegrationTests : IClassFixture<RelayWebApplicationFactory>, IDisposable
{
    private readonly RelayWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public AdminPolicyBundleIntegrationTests(RelayWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task Get_admin_policy_returns_bundle_shape()
    {
        using var res = await _client.GetAsync("/admin/policy");
    ...
    [Fact]
    public async Task Put_admin_policy_invalid_json_returns_400()
    {
    ...
    [Fact]
    public async Task Put_admin_policy_roundtrip_then_aggregated_ok()
    {
        using var getRes = await _client.GetAsync("/admin/policy");
        ...
        using var agg = await policyClient.GetAsync("/policy/aggregated");
