...
public sealed class TelemetryEffectiveEventKindsTests
{
    ...
    {
        var relay = R("a", "b");
        var e1 = TelemetryEffectiveEventKinds.Compute(relay, null);
        Assert.Equal(2, e1.Count);
        Assert.Contains("a", e1);
        var e2 = TelemetryEffectiveEventKinds.Compute(relay, new HashSet<string>(StringComparer.Ordinal));
    ...
    public void Compute_intersection_ignores_unknown_user_kinds()
    {
        ...
        var eff = TelemetryEffectiveEventKinds.Compute(relay, user);
        Assert.Single(eff);
    ...
    {
        var eff = TelemetryEffectiveEventKinds.Compute(new HashSet<string>(StringComparer.Ordinal), R("a"));
        Assert.Empty(eff);
    }
}
