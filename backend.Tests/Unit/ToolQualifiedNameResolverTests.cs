...
public class ToolQualifiedNameResolverTests
{
    ...
    private static ToolRegistry CreateRegistry()
    {
        ...
        reg.Register("Browser", "run_page_script", StubFn("run_page_script"));
    ...
    {
        var reg = CreateRegistry();
        Assert.True(ToolQualifiedNameResolver.TryResolve(reg, "RUN_PAGE_SCRIPT", out var p, out var bare, out _));
        Assert.Equal("Browser", p);
        Assert.Equal("RUN_PAGE_SCRIPT", bare);
    }
