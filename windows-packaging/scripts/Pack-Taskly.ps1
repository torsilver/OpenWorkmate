...
<#
.SYNOPSIS
  一键将 Taskly 整项目打入 windows-packaging/stage，并默认编译 Windows 安装包（MSI）。

.PARAMETER BuildInstaller
  默认 `$true`：在 stage 就绪后调用 WiX 编译 `installer/Taskly.wxs`，输出 `dist/Taskly.msi`。传 `-BuildInstaller:$false` 可跳过 MSI，仅保留 stage。
...
Invoke-Step "dotnet publish Taskly.StaticHost" {
  $out = Join-Path $stageRoot 'Taskly.StaticHost'
...
if (-not $SkipChrome) {
  ...
  New-Item -ItemType Directory -Force -Path (Join-Path $stageRoot 'taskly-chrome') | Out-Null
}
...
Invoke-Step "Generate Office manifest + StaticHost appsettings.Pack.json" {
  ...
  $staticDir = Join-Path $stageRoot 'Taskly.StaticHost'
  $appSettings = @{
    StaticHost = @{
      Port       = "$StaticHostPort"
      OfficeRoot = "..\\office-addin"
      WpsRoot    = "..\\wps-addin"
      ChromeRoot = "..\\taskly-chrome"
    }
  } | ConvertTo-Json -Depth 5
  Set-Content -Path (Join-Path $staticDir 'appsettings.Pack.json') -Value $appSettings -Encoding UTF8

  $idPath = Join-Path $stageRoot 'taskly-chrome\extension-id.txt'
  if (Test-Path $idPath) {
    $eid = (Get-Content $idPath -Raw).Trim()
    $setPol = Join-Path $stageRoot 'policies\Set-TasklyChromeExtensionPolicy.ps1'
    $remPol = Join-Path $stageRoot 'policies\Remove-TasklyChromeExtensionPolicy.ps1'
    $updUrl = "https://localhost:$StaticHostPort/taskly-chrome/updates.xml"
    ...
    [void]$sb.AppendLine('')
    [void]$sb.AppendLine('Remove Taskly policy entry only:')
    ...
    $installChrome = @"
# Apply Chrome ExtensionInstallForcelist for this install (run after Start Taskly).
`$ErrorActionPreference = 'Stop'
`$root = Split-Path -Parent `$MyInvocation.MyCommand.Path
`$eid = (Get-Content (Join-Path `$root 'taskly-chrome\extension-id.txt') -Raw).Trim()
`$set = Join-Path `$root 'policies\Set-TasklyChromeExtensionPolicy.ps1'
& `$set -ExtensionId `$eid -UpdatesXmlUrl 'https://localhost:$StaticHostPort/taskly-chrome/updates.xml'
...
"@
  ...
  $clientSetupSrc = Join-Path $packagingRoot 'templates\Taskly-ClientSetup.html'
...
if ($BackendTargetFramework -eq 'net10.0-windows') {
  $startPs1 = @"
# Generated by Pack-Taskly.ps1: start services without flashing console windows
`$ErrorActionPreference = 'Stop'
`$root = Split-Path -Parent `$MyInvocation.MyCommand.Path
Set-Location `$root
...
Set-Content -Path (Join-Path $stageRoot 'Start-Taskly.ps1') -Value $startPs1 -Encoding UTF8

Write-Host ""
Write-Host "Pack complete. Stage: $stageRoot" -ForegroundColor Green
Write-Host "Run locally:  powershell -NoProfile -ExecutionPolicy Bypass -File `"$stageRoot\Start-Taskly.ps1`"" -ForegroundColor Cyan
