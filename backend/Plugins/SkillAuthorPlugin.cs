...
## 内容质量要求
- 正文中应 **显式列出推荐使用的内置插件名**（须与系统中实际名称一致），例如 Excel、Word、Ppt、Browser、File、Memory、Plan、MCP_OCR、CurrentDocument、Tavily、ClawhubSkill 等；需要时用 `插件名-函数名` 或说明可插入 `[TOOL:插件名]` 以便后续对话中工具选择命中。**不要编造不存在的插件名。**
